...
def schedule_workers(function, args, max_processes=mp.cpu_count(), with_kwargs=False):
    '''
        ...
        for p, proc in processes.iteritems():
            if not proc or not proc.is_alive():
                if proc:
                    ...
                    proc.terminate()
                if with_kwargs:
                    new_proc = mp.Process(target=function, args=args[0], kwargs=args[1])
                else:
                    new_proc = mp.Process(target=function, args=args)
                new_proc.start()
