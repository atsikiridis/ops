...
class WebProfilePage():
    ...
    def __init__(self, page, heading, no_cache=False):
        ...
        self.stylesheets = self.stylesheets + ["%s/%s" % (self.legacy_css_dir, item) for item in self.legacy_stylesheets]
    ...
    def render_citations_summary_content(citations, canonical_name):
        ...
        for category in citations['breakdown_categories']:
            ...
            if len(limits) > 1 and limits[0] is not 0:
                ...
                high = 1000000
            citeable_breakdown_queries[category] = tmpl_citesummary_get_link_for_rep_breakdown(canonical_name, 'author', 'collection:citeable','cited',low,high)
            published_breakdown_queries[category] = tmpl_citesummary_get_link_for_rep_breakdown(canonical_name, 'author', 'collection:published','cited',low,high)
...
#            open_rt_tickets = [a for a in open_rt_tickets if a[1] == rt_tickets]
            ...
            for t in open_rt_tickets:
                ...
                for a in actions:
                    ...
                    h(' - <a rel="nofollow" id="show_paper" target="_blank" href="%(url)s/record/%(record)s"> View record </a><br>' % ({'url':CFG_SITE_URL, 'record':str(bibrec)}))
                h('</dd>')
                h('</dd><br>')
...
You may also continue as a guest. In this case your input will be processed by our staff and will take longer to display."><strong> Login with your arXiv.org account </strong></span>""")
        ...
        if arxiv_data['login'] == True:
            if arxiv_data['view_own_profile'] == True:
                ...
                html_arxiv = _("You have succesfully logged in via arXiv.<br><font color='red'>However the profile you are viewing is not your profile.<br><br></font>")
                own_profile_link = "%s/author/manage_profile/%s" % (CFG_SITE_URL, arxiv_data['user_pid'])
                own_profile_text = _("Manage your profile")
                html_arxiv += '<a rel="nofollow" href="%s" class="btn btn-default">%s</a>' % (own_profile_link, own_profile_text)
            else:
                html_arxiv = _("You have succesfully logged in, but<font color='red'> you are not associated to a person yet.<br>Please use the button below to choose your profile<br></font>")
...
It holds a record of all your research activities. You can add your ORCiD to all your works to make sure they are associated with you. ">
        ...
        if orcid_data['orcids']:
            html_orcid += _('This profile is already connected to the following ORCiD: <strong>%s</strong></br>' % (",".join(['<a rel="nofollow" href="http://www.orcid.org/' + orcidid +'"">' + orcidid + '</a>' for orcidid in orcid_data['orcids']]),))
            if orcid_data['arxiv_login'] and orcid_data['own_profile']:
                html_orcid += '<br><a rel="nofollow" href="%s" class="btn btn-default">%s</a>' % ("%s/author/manage_profile/import_orcid_pubs" % CFG_SITE_SECURE_URL, _("Import your publications from ORCID") )
                html_orcid += '<br><br><a rel="nofollow" href="http://orcid.org/%s" class="btn btn-default">%s</a>' % (orcid_data['orcids'][0], _("Visit your profile in ORCID") )
        else:
            ...
            if orcid_data['arxiv_login'] and (orcid_data['own_profile'] or orcid_data['add_power']):
                ...
                html_orcid += '<br><br><a rel="nofollow" href="%s" class="btn btn-default">%s</a>' % (add_link, add_text)
            else:
                ...
                #html_orcid += '<input type="submit" class="btn btn-default" value="%s"> </form>' % ('Submit suggestion',)

                #html_orcid += '<a rel="nofollow" href="%s" class="btn btn-default">%s</a>' % (suggest_link, suggest_text)
...
You can also assign publications to other authors. This will help %s provide more accurate publication and citation statistics. "><strong> Manage publications </strong><span>""" % BIBAUTHORID_CFG_SITE_NAME)
        ...
        html_claim_paper += 'Assign publications to your %s profile to keep it up to date. </br></br> <a rel="nofollow" href="%s" class="btn btn-default">%s</a>' % (BIBAUTHORID_CFG_SITE_NAME, link, text)
    ...
    def tmpl_ext_ids_box(self, personid,  int_ids_data, ext_ids_data, ln, add_box=True, loading=True):
        ...
        if 'ext_ids' in ext_ids_data and ext_ids_data['ext_ids']:
            ...
            html_ext_ids += '        <br> <br> <input type="submit" class="btn btn-default" value="%s"> <br> </form>' % (remove_text,)
        else:
        ...
        html_ext_ids += '   <input type="hidden" name="pid" value="%s">' % ext_ids_data['person_id']
        html_ext_ids += '   <input type="submit" class="btn btn-default" value="%s"> </form>' % ('Set (steal!) user id',)
        html_ext_ids += '</div>'
...
This list contains all your publications that were automatically assigned to your INSPIRE profile through arXiv and ORCiD. "><strong> Automatically assigned publications </strong> </span>""")
        ...
        if loading:
            ...
            if "unsuccessfull_recids" in autoclaim_data.keys() and autoclaim_data["unsuccessfull_recids"]:
                ...
                html_autoclaim += '<br><span class=\"bsw\"><a rel="nofollow" href="%s" class="btn btn-default">%s</a></span><br><br>' % (link, text)
...
After the merging only the primary profile will exist in the system and all others will be automatically deleted. "><strong> Merge profiles </strong><span>""")
        html_merge = _("If your or somebody else's publications in %s exist in multiple profiles, you can fix that here. </br>" % BIBAUTHORID_CFG_SITE_NAME)
        ...
        if not loading:
            ...
            if hepnames_data['HaveHep']:
                ...
                if not hepnames_data['HaveChoices']:
                    ...
                    mailstr = ('''<a id="hepname_connection" class="btn btn-default" href="#"><span class="cname hidden">%s</span><span class="hepname hidden">%s</span>%s</a>''' % (hepnames_data['cid'], '%s', 'This is the right one!'))
